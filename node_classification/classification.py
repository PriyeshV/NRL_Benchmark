...
This script is built on the scoring script used in the Deepwalk Paper.
---- Can be found here:
...
def calc_metrics(normalized, dataset, algorithm, num_shuffles, all_results, best_classifiers, writetofile, embedding_params, emb_size, output_dir_and_classifier_name):

    columns=["Algorithm", "Dataset", "Train %", "Normalized Embeddings", "Micro-F1", "Macro-F1", "Accuracy", "Num of Shuffles", "Embedding Size"]
    ...
    for train_percent in sorted(all_results.keys()):
        ...
        if embedding_params != None:
            ...
            print ("{},{},{},{},{},{},{},{},{},{}".format(algorithm, dataset, normalized, train_percent, avg_score["micro"], avg_score["macro"], avg_score["accuracy"], num_shuffles, emb_size, embedding_params_string))
        else:
            print ("{},{},{},{},{},{},{},{},{}".format(algorithm, dataset, normalized, train_percent, avg_score["micro"], avg_score["macro"], avg_score["accuracy"], num_shuffles, emb_size))

    if writetofile:
        output_file_name = os.path.join(output_dir_and_classifier_name + "_" + dataset + "_" + "classi_results" + "_" + algorithm + embedd_params_file_name + ".csv")
...
def validate_inputs(emb, network, training_percents, dataset, algorithm, writetofile):
    if emb == "":
        print ("Missing embedding file path.")
        sys.exit(1)
    elif (os.path.exists(emb) == False):
        print ("Embedding file path does not exist.")
        sys.exit(1)
    if network == "":
...
def classify(emb="", network="", training_percents=training_percents_default, dataset="", writetofile=True, algorithm="", adj_matrix_name="network", label_matrix_name="group", num_shuffles=10, word2vec_format=True, embedding_params=None, classifier="LR",  test_kernel="linear", grid_search=True, output_dir="./"):
    ...
    validate_inputs(emb, network, training_percents, dataset, algorithm, writetofile)
    ...
    for features_matrix_tuple in feature_matrices:
        ...
        for train_percent in training_percents:
            ...
            for x in range(num_shuffles):
                ...
                del X_train, X_test, y_train_, y_train, y_test_, y_test, shuf, clf
            logger.info("Done with %s", train_percent)

        calc_metrics(normalized, dataset, algorithm, num_shuffles, all_results, best_classifiers, writetofile, embedding_params, X.shape[1], output_dir + classifer_name_for_file)
